...
active_lot_stats AS (
    ...
    GROUP BY l.production_id, li.product_id, li.unit, p.weight_kg, li.lot_id -- Group thêm lot_id để tránh sum sai nếu li có nhiều record cho 1 unit? Không, group theo unit là đủ.
),
-- Nguồn 2: export_task_items cho lot đã xuất hết
...
combined_stats AS (
    -- Group lại một lần nữa theo production_id + product_id + unit để merge nguồn
    SELECT 

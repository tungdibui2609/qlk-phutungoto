-- Generically find and fix ALL references to 'branches' to allow deletion

DO $$
DECLARE
    r RECORD;
BEGIN
    -- Loop through all Foreign Keys referencing public.branches
    FOR r IN 
        SELECT 
            tc.constraint_name, 
            tc.table_name, 
    ...
    LOOP
        ...
        BEGIN
            ...
            EXECUTE format('ALTER TABLE public.%I ADD CONSTRAINT %I FOREIGN KEY (%I) REFERENCES public.branches(%I) ON DELETE CASCADE', 
                r.table_name, r.constraint_name, r.column_name, parent_col);
                
            RAISE NOTICE 'Updated % . % to CASCADE (Referencing branches . %)', r.table_name, r.column_name, parent_col;
        END;

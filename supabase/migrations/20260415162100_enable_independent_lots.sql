-- Enable Independent Lots for Production
-- 1. Add production_lot_id to lots table
ALTER TABLE public.lots 
ADD COLUMN IF NOT EXISTS production_lot_id UUID REFERENCES public.production_lots(id) ON DELETE SET NULL;

COMMENT ON COLUMN public.lots.production_lot_id IS 'Liên kết trực tiếp với mã slot sản xuất (production_lots). Cho phép tách biệt sản lượng khi một sản phẩm chạy nhiều lô.';

-- 2. Update production_item_statistics VIEW to group by production_lot_id accurately
DROP VIEW IF EXISTS public.production_item_statistics;

CREATE OR REPLACE VIEW public.production_item_statistics AS
WITH 
-- Nguồn 1: lot_items (vật lý, đang tồn trong kho hoặc ghi nhận lúc nhập)
active_lot_stats AS (
    SELECT 
        l.production_id,
        l.production_lot_id,

        ...
        >
            <div className="p-3 flex flex-col h-full">
                ...
                <div className="flex gap-3 items-stretch flex-1 min-h-0">
                    {/* QR Code Segment (45%) */}
                    <div className="w-[45%] flex flex-col items-center justify-center border-r border-dashed border-black pr-2">
                        <div className="bg-white p-1">
                            <QRCode value={data.scan_url} size={scale < 0.5 ? 85 : 120} />
                        </div>
                        <p className="text-[7px] mt-1.5 font-black tracking-[0.2em] text-black whitespace-nowrap">SCAN TO ASSIGN</p>
                    </div>
                    ...
                    <div className="w-[55%] flex flex-col justify-between py-0.5 min-w-0">
                        <div className="min-w-0">
                            <div className="flex justify-between items-end mb-1">
                                <p className="text-[7px] font-bold uppercase text-black leading-none">Sản phẩm</p>
                                {data.products?.[0]?.sku && (
                                    <p className="text-[7px] font-black text-black leading-none">CODE: {data.products?.[0]?.sku}</p>
                                )}
                            </div>
                            ...
                            {(data.products?.[0]?.tags && data.products?.[0]?.tags.length > 0) && (
                                <div className="flex flex-wrap justify-center gap-1 mt-1.5">
                                    {data.products?.[0]?.tags.map((tag: string, i: number) => (
                                        <span key={i} className="text-[7px] px-1 font-bold border border-black rounded-[2px] bg-white text-black shrink-0">
                        ...
                        <div className="border-t border-black pt-1.5 flex flex-col items-center">
                            <p className="text-[7px] font-bold uppercase text-black mb-1 leading-none">Qty / Số lượng</p>

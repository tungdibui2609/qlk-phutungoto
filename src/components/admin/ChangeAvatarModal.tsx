...
export default function ChangeAvatarModal({ onClose }: ChangeAvatarModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // 1. Upload to API (Cloudinary)
            const formData = new FormData()
            formData.append('file', file)
            formData.append('filename', `admin_avatar_${profile.id}`)
            ...
            // 3. Update user_profiles table (Directly, though updateProfileSettings in context could also work)
            ...
            setTimeout(() => {
                onClose()
                router.refresh() // Force Next.js to refresh server components/layout
            }, 1000)
            ...
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="flex flex-col items-center gap-4">
                        <div
                            className="relative w-32 h-32 rounded-full border-4 border-stone-100 overflow-hidden cursor-pointer group shadow-inner"
                        ...
                        <div className="text-center">
                            <button
                                type="button"
                                onClick={() => fileInputRef.current?.click()}
                                className="text-sm font-medium text-orange-600 hover:text-orange-700 underline decoration-dashed underline-offset-4"
                            >
                                Chọn ảnh mới
                            </button>
                    ...
                    <div className="pt-2 flex justify-end gap-3 border-t border-stone-100 mt-6">
                        ...
                        <button
                            ...
                            className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 shadow-lg shadow-orange-200 transition-all disabled:opacity-70 flex items-center gap-2"
                        >

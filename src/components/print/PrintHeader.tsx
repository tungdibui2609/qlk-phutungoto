...
export function PrintHeader({
    ...
    return (
        <div className={`relative ${isA5 ? 'mt-1 mb-0' : 'mb-4'} ${className}`} style={{ fontFamily: "'Times New Roman', Times, serif" }}>
            ...
            <div className={`flex items-center ${isA5 ? 'gap-2.5' : 'gap-3'}`}>
                ...
                <div className="shrink-0">
                    {logoSrc || companyInfo?.logo_url ? (
                        <img
                            ...
                            alt="Logo"
                            className={isA5 ? (isLarge ? "h-12 w-auto object-contain" : "h-9 w-auto object-contain") : (isLarge ? "h-20 w-auto object-contain" : "h-10 w-auto object-contain")}
                        />
                    ) : (
                        <div className={`${isA5 ? (isLarge ? 'h-12 w-12 text-lg' : 'h-9 w-9 text-sm') : (isLarge ? 'h-20 w-20 text-2xl' : 'h-14 w-14 text-xl')} bg-orange-500 rounded-lg flex items-center justify-center text-white font-bold`}>
                ...
                <div className={`flex flex-col justify-center ${isA5 ? 'gap-0' : 'gap-0.5'} ${isA5 ? 'min-w-0 flex-1' : ''}`}>
                    <div className={`text-black font-bold uppercase leading-tight ${isA5 ? (isLarge ? 'text-[12px]' : 'text-[10px]') : (isLarge ? 'text-sm' : 'text-[10px]')} mb-0 ${isA5 ? '' : 'whitespace-nowrap'}`}>
                        {companyInfo?.name || 'CÔNG TY'}
                    </div>
                    {companyInfo?.address && (
                        <div className={`font-bold text-black leading-tight ${isA5 ? (isLarge ? 'text-[10px]' : 'text-[9px]') : (isLarge ? 'text-sm' : 'text-[8px]')}`}>

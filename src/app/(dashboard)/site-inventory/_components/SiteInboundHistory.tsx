...
export const SiteInboundHistory = () => {
    ...
    const fetchHistory = async () => {
        setLoading(true)
        try {
            const data = await loanService.getInboundHistory(supabase, systemType!)
            setHistory(data || [])
        } catch (error) {
            console.error(error)
        } finally {
        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-white dark:bg-zinc-800 rounded-[32px] border border-dashed border-stone-200 dark:border-zinc-700">
                    ...
                    <h3 className="text-lg font-bold text-stone-500 dark:text-zinc-400">Chưa có lịch sử nhập hàng trực tiếp</h3>

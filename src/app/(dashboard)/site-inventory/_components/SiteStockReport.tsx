...
import { Package, Hash, Warehouse, HardHat, FileText, Search, RefreshCw, Layers } from 'lucide-react'
...
export const SiteStockReport = () => {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const data = await loanService.getSiteInventorySummary(supabase, systemType!)
            setSummary(data)
        } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 bg-orange-50/50 dark:bg-orange-900/10 rounded-2xl border border-orange-100 dark:border-orange-900/20">
                <p className="text-xs text-orange-700 dark:text-orange-400 flex items-center gap-2 font-medium">
                    <FileText size={14} /> 
                    Ghi chú: Báo cáo này tổng hợp dữ liệu từ "Hàng lẻ tại công trình" (Kho) và "Sổ mượn đồ" (Đang cấp phát). 
                    Giúp quản lý biết chính xác tổng số lượng vật tư thực tế đang có mặt tại hiện trường.
                </p>
            </div>
        </div>
    )

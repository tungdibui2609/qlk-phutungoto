...
import { Factory, Calendar, Clock, User, CheckCircle2, XCircle, AlertTriangle, FileText, ArrowLeft, RefreshCw, BarChart3, TrendingUp, Package, Shield, Layers, HelpCircle, Trash2, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function DeliveryShiftsPage() {
    ...
    // Load các sub-shifts (chốt tạm) của ca đang chọn
    useEffect(() => {
        const loadSubShifts = async () => {
            if (!selectedShift || !currentSystem) {
                setSubShifts([])
                return
            }
        ...
        loadSubShifts()
    }, [selectedShift, currentSystem])

    // Tự động load settings map để phân rã mo_code
    useEffect(() => {
        ...
        loadSettingsMap()
    }, [currentSystem?.company_id, profile?.company_id])

    // Load các đợt giao nhận thuộc ca làm việc đang chọn
    useEffect(() => {
        ...
        loadShiftJournals()
    }, [selectedShift, currentSystem?.company_id, profile?.company_id])

    // Tự động gom nhóm Lệnh sản xuất từ danh sách journal
    const getMoSummary = () => {
    ...
    }, [currentSystem, profile, selectedShift])

    useEffect(() => {
        loadShifts()
    }, [currentSystem?.code, currentSystem?.company_id, profile?.company_id])
                ...
                {moSummary && Object.keys(moSummary).length > 0 ? (
                    <div>
                        <h5 className="text-[11px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                            <Layers size={12} className="text-orange-500" /> Đối soát chi tiết theo Lệnh sản xuất (MO)
                        </h5>
                        <div className="space-y-3">
                            {Object.entries(moSummary).map(([moCode, moData]: any) => (
                                <div key={moCode} className="bg-stone-50/60 dark:bg-zinc-900/40 border border-stone-200/50 dark:border-zinc-700/40 rounded-xl p-3 shadow-xs">
                                    <div className="text-xs font-black text-indigo-600 dark:text-indigo-400 mb-2 flex items-start sm:items-center justify-between flex-col sm:flex-row gap-2">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <div className="flex items-center gap-2 text-stone-500 dark:text-stone-400 text-xs font-semibold uppercase tracking-wider mb-1">
                        ...
                        <span>{currentSystem?.name || 'Phân hệ kho'}</span>
                        <span>•</span>
                        <span>Ca làm việc</span>
                    </div>
                    <h1 className="text-2xl font-black text-stone-900 dark:text-white flex items-center gap-2">
                        <Clock className="text-orange-500" /> Lịch Sử Ca & Đối Soát
                    </h1>
                </div>
                <div className="flex items-center gap-2">
                    <Link 
                        href="/delivery-journal"
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`p-4 cursor-pointer hover:bg-stone-50/50 dark:hover:bg-zinc-700/30 transition-all border-l-4 ${
                                            isSelected 
                                                ? 'bg-orange-50/30 dark:bg-orange-950/10 border-l-orange-500' 
                                                : s.status === 'open' 
                        ...
                        <div>
                            ...
                            <div className="p-4 md:p-6 space-y-6">
                                ...
                                <div className="border-t border-stone-200/50 dark:border-zinc-700/50 pt-6">
                                    ...
                                    >
                                        <div className="flex items-center gap-1.5">
                                            <BarChart3 size={16} className="text-orange-500 transition-transform group-hover:scale-110" />
                                            <span className="group-hover:text-orange-500 transition-colors">Tổng hợp đối soát cả ca (Sum tổng)</span>
                                        </div>
                                        <div className="flex items-center gap-2">

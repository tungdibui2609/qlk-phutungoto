...
import { Factory, Search, Send, Check, PackageOpen, ClipboardCheck, Truck, RefreshCw, X, AlertTriangle, ArrowLeftRight, Bell, Hash, Camera, Loader2, ArrowLeft, Pencil } from 'lucide-react'
...
export default function DeliveryJournalSinglePage() {
    ...
    // Modal Giao vật tư trực tiếp (Kho -> SX)
    ...
    // Modal Nhận thành phẩm trực tiếp (SX -> Kho)
    const [receiveModal, setReceiveModal] = useState<{ setting: DeliverySetting } | null>(null)
    const [receiveQty, setReceiveQty] = useState(0)
    const [receiveUnit, setReceiveUnit] = useState('Thùng')
    ...
    // Giao nhận dở dang (vẫn hiển thị để xử lý nếu có phát sinh từ luồng 2 bên)
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('delivery_journal_single')
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
    ...
    // Xử lý Giao vật tư trực tiếp (Kho -> SX)
    const handleSendDirect = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                status: 'received_by_production', // Trạng thái hoàn thành ngay lập tức
                notes: sendNotes || setting.notes || `[Giao nhận trực tiếp] Lệnh: ${setting.mo_code}`,
                
                // Điền thông tin gửi và nhận đồng thời bằng tài khoản hiện tại
                sent_by: profile?.id || null,
                sent_by_name: profile?.full_name || 'Nhân viên kho',
                sent_at: nowStr,
                
                received_by_production: profile?.id || null,
                received_by_production_name: profile?.full_name || 'Nhân viên kho',
                received_by_production_at: nowStr,
            ...
            alert('Lỗi giao vật tư: ' + (err?.message || err))
        }
    }

    // Xử lý Nhận thành phẩm trực tiếp (SX -> Kho)
    const handleReceiveDirect = async () => {
        if (!receiveModal || !currentSystem) return
        const { setting } = receiveModal
        try {
            const nowStr = new Date().toISOString()
            const payload = {
                system_code: currentSystem.code,
                company_id: currentSystem.company_id || profile?.company_id || null,
                setting_id: setting.id,
                item_name: setting.product_name, // Tên thành phẩm
                quantity_sent: receiveQty || 1,  // Số lượng SX giao đi
                unit: receiveUnit,
                from_department: 'Sản xuất',
                to_department: 'Kho',
                status: 'received_by_warehouse', // Trạng thái hoàn thành ngay lập tức
                notes: receiveNotes || `[Giao nhận trực tiếp] Nhận thành phẩm Lệnh: ${setting.mo_code}`,
                ...
                completed_by_name: profile?.full_name || 'Nhân viên kho',
                completed_by_production_at: nowStr,
                
                received_by_warehouse: profile?.id || null,
                received_by_warehouse_name: profile?.full_name || 'Nhân viên kho',
                received_by_warehouse_at: nowStr,
                
                result_item_name: setting.product_name,
                result_quantity: receiveQty || 1, // Số lượng kho nhận
                result_unit: receiveUnit,
            ...
            console.error('Receive direct error:', err)
            alert('Lỗi nhận thành phẩm: ' + (err?.message || err))
        }
    }

    // Nhận hộ các đợt dở dang nếu có từ luồng 2 bên
    const handleDirectReceive = async (j: DeliveryJournal) => {
        ...
        try {
            ...
            if (isFromProductionSent) {
                updatePayload.result_quantity = j.result_quantity || j.quantity_sent
    ...
    const handleDeleteJournal = async (id: string) => {
        if (!currentSystem) return
        if (!confirm('Bạn có chắc chắn muốn xóa đợt giao nhận này không?')) return
        ...
        try {
            ...
            alert('Lỗi xóa đợt giao nhận: ' + (err?.message || err))
        }
    }
    ...
    const openReceiveModal = (setting: DeliverySetting) => {
        setReceiveModal({ setting })
        setReceiveQty(1)
        setReceiveUnit('Thùng')
        setReceiveNotes(`Nhận thành phẩm Lệnh: ${setting.mo_code}`)
    }
    ...
    if (!hasModule('delivery_journal')) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
                ...
                <h2 className="text-2xl font-bold text-stone-800 dark:text-stone-100 mb-2">Giao nhận Trực tiếp (1 bên)</h2>
    ...
    return (
        <div className="h-[calc(100vh-4rem)] flex flex-col bg-stone-50 dark:bg-zinc-900">
                                    ...
                                    {notifications.map(n => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p className="text-xs text-amber-600">{n.count} đợt SX đã gửi, chờ nhận</p>
                                            </button>
                                        )
                                    })}
                                </div>
                    ...
                    <div>
                        <div className="bg-emerald-50 dark:bg-emerald-950/20 border-b border-emerald-100 dark:border-emerald-900/30 px-3 py-2 sm:px-6 sm:py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                            ...
                            <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-2">
                                ...
                                <Link href="/delivery-shifts" className="text-stone-500 hover:text-stone-700 dark:text-stone-400 dark:hover:text-stone-300 text-[10px] sm:text-[11px] font-bold underline">
                                    Lịch sử
                                </Link>
                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            console.error('Error checking pending journals for full close:', err)
                                            alert('Lỗi kiểm tra đợt giao nhận dở dang')
                                        }
                                    }}
                ...
                <div className={`w-full lg:flex-1 overflow-y-auto p-4 lg:p-6 ${selectedMoId ? 'flex' : 'hidden lg:flex'} flex-col`}>
                    {!selectedGroup ? (
                        <div className="flex flex-col items-center justify-center h-full text-center py-20">
                            ...
                            <p className="text-stone-400 text-sm mt-1">để thao tác giao nhận trực tiếp</p>
                        </div>
                    ) : (
                        <>
                            {/* Mobile Back Button */}
                                            ...
                                            <div key={setting.id} className={`rounded-2xl border shadow-sm overflow-hidden transition-all duration-350 ${
                                                ...
                                                <div className={`p-3 sm:p-4 border-b border-stone-100 dark:border-zinc-700 transition-colors ${
                                                    ...
                                                    <div className="flex items-start justify-between gap-3">
                                                        <div className="min-w-0 flex-1">
                                                            <h3 className="font-extrabold text-[11px] sm:text-sm md:text-base text-stone-800 dark:text-stone-100 leading-snug break-words">
                                                                {setting.product_name}
                                                            </h3>
                                                            <div className="flex flex-wrap items-center gap-1 mt-1 sm:mt-1.5">
                                                                <span className="text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded bg-stone-100 dark:bg-zinc-700 text-stone-500 shrink-0">
                                                                    {dir}
                                                                </span>
                                                                <span className="text-[8px] sm:text-[9px] text-stone-500 dark:text-stone-400 font-medium">
                                                                    Cấu hình: {setting.quantity} {setting.unit}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        {activeShift ? (
                                                            <div className="flex items-center gap-1.5 shrink-0">
                                                                <button 
                                                                    onClick={() => openSendModal(setting)} 
                                                                    className="flex items-center gap-0.5 px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-[9px] sm:text-xs font-black shadow-md shadow-indigo-500/20 active:scale-95 transition-all shrink-0"
                                                                >
                                                                    <Send size={10} /> ⚡ Giao đi (Kho → SX)
                                                                </button>
                                                                <button 
                                                                    onClick={() => openReceiveModal(setting)} 
                                                                    className="flex items-center gap-0.5 px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[9px] sm:text-xs font-black shadow-md shadow-emerald-500/20 active:scale-95 transition-all shrink-0"
                                                                >
                                                                    <Check size={10} /> ⚡ Nhận về (SX → Kho)
                                                                </button>
                                                            </div>
                                                        ) : (
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-1.5 sm:gap-x-2.5 gap-y-1 mt-2 text-[9px] sm:text-xs font-bold text-stone-500 dark:text-stone-400">
                                                        <span>Đã giao nhận: <strong className="text-stone-700 dark:text-stone-300">{getSummaryText(journals, j => j.status !== 'cancelled', setting.unit)}</strong></span>
                                                        <span className="text-stone-300 dark:text-zinc-700">•</span>
                                                        <span>Thành công: <strong className="text-emerald-600 dark:text-emerald-400">{getSummaryText(journals, j => j.status === 'received_by_warehouse' || j.status === 'received_by_production', setting.unit)}</strong></span>
                                                        {waitCount > 0 && (
                                                            <>
                                                                <span className="text-stone-300 dark:text-zinc-700">•</span>
                                                                <span className="text-rose-600 dark:text-rose-400 animate-pulse flex items-center gap-0.5">
                                                                    <AlertTriangle size={10} className="shrink-0" /> {waitCount} chờ nhận (luồng 2 bên)
                                                                </span>
                                                            </>
                                                        )}
                                                    </div>
                                                ...
                                                {/* === ĐỢT CHỜ XỬ LÝ (Từ luồng 2 bên phát sinh, vẫn cho xử lý để không bị lỗi ca) === */}
                                                {(() => {
                                                    const pendingJournals = journals.filter(j => 
                                                        j.status === 'completed_by_production' || (j.status === 'sent' && (j as any).from_department === 'Sản xuất')
                                                    )
                                                        ...
                                                        <div className="border-t border-rose-200 dark:border-rose-900/30 bg-rose-50/60 dark:bg-rose-950/10">
                                                            <div className="px-3 py-1.5 sm:px-4 sm:py-2">
                                                                <span className="text-[9px] sm:text-[10px] font-extrabold text-rose-600 dark:text-rose-400 uppercase tracking-wider flex items-center gap-1">
                                                                    <AlertTriangle size={10} className="animate-pulse" /> {pendingJournals.length} đợt 2 bên chờ Kho nhận
                                                                </span>
                                                            </div>
                                                            <div className="divide-y divide-rose-100 dark:divide-rose-900/20">
                                                                {pendingJournals.map(j => {
                                                                    const st = j.status
                                                                    let badge = st === 'completed_by_production' 
                                                                        ? <span className="text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 font-black">SX xong</span>
                                                                        : <span className="text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 font-black">SX gửi</span>
                                                                    return (
                                                                        <div key={j.id} className="flex items-center justify-between gap-2 px-3 py-2 sm:px-4 sm:py-2.5 bg-rose-100/50 dark:bg-rose-900/20 border-l-[3px] border-l-rose-500">
                                                                            <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                                                                                ...
                                                                                {(st === 'completed_by_production' || st === 'received_by_warehouse') && j.result_item_name && (
                                                                                    <span className="text-[8px] sm:text-[9px] text-emerald-600 font-medium">→ {j.result_item_name}</span>
                                                                                )}
                                                                            </div>
                                                                            <div className="flex items-center gap-1.5 shrink-0">
                                                ...
                                                {journals.length > 0 && (
                                                    ...
                                                    >
                                                        <span className="flex items-center gap-1">
                                                            📋 Lịch sử giao nhận ({journals.length} đợt trực tiếp)
                                                        </span>
                                                            ...
                                                            <div className="divide-y divide-stone-100 dark:divide-zinc-700 max-h-60 overflow-y-auto">
                                                                {[...journals]
                                                                    ...
                                                                    .map(j => {
                                                                        ...
                                                                        if (st === 'sent') {
                                                                            if ((j as any).from_department === 'Kho') badge = <span className="text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 font-black">Kho gửi</span>
                                                                            else badge = <span className="text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 font-black">SX gửi</span>
                                                                        } else if (st === 'received_by_production') {
                                                                            badge = <span className="text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-black">✓ Đã giao trực tiếp</span>
                                                                            ...
                                                                            badge = <span className="text-[8px] sm:text-[9px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-black">✓ Đã nhận trực tiếp</span>
                                                                        ...
                                                                        return (
                                                                            <div key={j.id} 
                                                                                ...
                                                                                className="flex flex-col cursor-pointer px-2 py-1.5 sm:px-4 sm:py-2 transition-all hover:bg-stone-50/50 dark:hover:bg-zinc-700/20"
                                                                            >
                                                                                ...
                                                                                {isExpanded && (
                                                                                    <div className="text-[10px] text-stone-400 mt-2 pl-2 border-l border-stone-200 dark:border-zinc-700 space-y-1 py-0.5 animate-fadeIn">
                                                                                        {j.sent_by_name && (
                                                                                            <div>Thao tác trực tiếp bởi: <span className="font-semibold text-stone-600 dark:text-stone-300">{j.sent_by_name}</span> lúc <span className="font-medium text-stone-500">{formatDateTime(j.sent_at)}</span></div>
                                                                                        )}
                                                                                        {(st === 'received_by_production' || st === 'received_by_warehouse') && (
                                                                                            <div className="text-emerald-600 dark:text-emerald-400 font-medium">Trạng thái: <span className="font-bold">Đã hoàn thành (1 bên tự ghi nhận)</span></div>
                                                                                        )}
                                                                                        {(st === 'received_by_warehouse') && j.result_item_name && (
                                                                                            <p className="text-[10px] text-emerald-600 font-medium mt-1">→ Đã nhập kho: {j.result_item_name} ({j.result_quantity} {j.result_unit})</p>
                                                                                        )}
                                                                                        {j.notes && (
                                                                                            <div className="text-[9px] text-stone-500 italic mt-1 pl-1">Ghi chú: {j.notes}</div>
                                                                                        )}
                                                                                        <div className="mt-2 flex items-center justify-end gap-2 border-t border-stone-150/40 dark:border-zinc-700/40 pt-2 animate-fadeIn">
                                                                                            <button
                                                                                                onClick={(e) => {
                                                                                                    e.stopPropagation()
                                                                                                    openEditModal(j)
                                                                                            ...
                                                                                            <button
                                                                                                ...
                                                                                                className="px-2.5 py-1 bg-red-50 hover:bg-red-100 dark:bg-red-950/20 text-red-600 rounded-lg text-[9px] font-bold flex items-center gap-1 transition-all active:scale-95 border border-red-200 dark:border-red-900/30"
                                                                                            >
                                                                                                <X size={9} className="text-red-500" /> Xóa đợt này
                                                                                            </button>
                                                                                        </div>
                                                                                    </div>
                                                                                )}
                    ...
                    <div className="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-fadeIn">
                        <div className="p-6">
                            ...
                            <div className="bg-stone-50 dark:bg-zinc-900 rounded-2xl p-3 mb-4">
                                ...
                                <p className="text-xs text-stone-500">Lệnh: {sendModal.setting.mo_code} | ĐVT mặc định: {sendModal.setting.unit}</p>
                            </div>

                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-bold text-stone-700 dark:text-stone-300 mb-1">Số lượng giao</label>
                                        <input
                                            type="number"
                                            min={0.01}
                                            step="any"
                            ...
                            <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-stone-200 dark:border-zinc-700">
                                ...
                                <button onClick={handleSendDirect} className="px-6 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-2xl shadow-lg shadow-indigo-500/20 active:scale-95 transition-all">
                                    ⚡ Xác nhận Giao &amp; Nhận
                                </button>
                            </div>
                        </div>
            ...
            {/* Modal Nhận thành phẩm trực tiếp */}
            {receiveModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-fadeIn">
                        <div className="p-6">
                            ...
                            <div className="bg-stone-50 dark:bg-zinc-900 rounded-2xl p-3 mb-4">
                                <p className="font-bold text-stone-800 dark:text-stone-200">{receiveModal.setting.product_name}</p>
                                <p className="text-xs text-stone-500">Lệnh: {receiveModal.setting.mo_code} | ĐVT mặc định: {receiveModal.setting.unit}</p>
                            </div>

                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-bold text-stone-700 dark:text-stone-300 mb-1">Số lượng nhận</label>
                                        <input
                                            type="number"
                                            min={0.01}
                                            step="any"
                            ...
                            <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-stone-200 dark:border-zinc-700">
                                ...
                                <button onClick={handleReceiveDirect} className="px-6 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-2xl shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">
                                    ⚡ Xác nhận Thu hồi &amp; Nhập kho
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                                        ...
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="bg-stone-100 dark:bg-zinc-800 font-bold text-stone-500 border-b border-stone-200/50">
                                                    ...
                                                    <th className="p-2 text-right">Giao nhận</th>
                                            ...
                                            <tbody>
                                                {Object.entries(interimSummary.units_summary).map(([unit, data]: any) => (
                                                    <tr key={unit} className="border-b border-stone-100 dark:border-zinc-700/30">
                                                        ...
                                                        <td className="p-2 text-right font-semibold text-stone-700 dark:text-stone-300">{data.sent}</td>
                                                        <td className="p-2 text-right font-semibold text-emerald-600">{data.received}</td>
                                                        <td className="p-2 text-right font-semibold text-rose-600">{data.cancelled}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                            ...
                            <div className="mb-4">
                                <label className="block text-xs font-bold text-stone-500 mb-1.5 uppercase">Ghi chú chốt ca tạm</label>
                                <input
                                    type="text"
                                    value={interimNotes}
                                    onChange={(e) => setInterimNotes(e.target.value)}
                                    placeholder="Không bắt buộc. Nhập ghi chú như: Bàn giao ca sáng, đổi nhân sự..."
                                    className="w-full px-4 py-2.5 bg-stone-50 dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl text-sm"
                                />
                            </div>

                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-stone-200 dark:border-zinc-700">
                                <button onClick={() => setInterimCloseModal(false)} className="px-4 py-2 text-sm font-bold text-stone-600 hover:bg-stone-100 dark:hover:bg-zinc-700 rounded-2xl">
                                    Hủy
                                </button>
                                <button onClick={handleInterimClose} className="px-6 py-2 text-sm font-bold text-white bg-amber-500 hover:bg-amber-600 rounded-2xl shadow-lg active:scale-95 transition-all">
                                    Xác nhận chốt tạm
                                </button>
                            </div>
                        </div>
            ...
            {/* Close Shift Modal (Chốt sổ ca chính) */}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            ...
                            <div className="bg-stone-50 dark:bg-zinc-900 rounded-2xl p-4 mb-4 space-y-3">
                                <h4 className="text-xs font-black text-stone-500 uppercase tracking-wider">Tổng kết ca làm việc (Toàn bộ ca)</h4>
                                <div className="grid grid-cols-3 gap-2 text-center">
                                    <div className="p-2 bg-white dark:bg-zinc-800 rounded-xl border border-stone-200/50">
                                        <div className="text-[9px] text-stone-400 font-bold uppercase">Tổng đợt</div>
                            ...
                            <div className="mb-4">
                                <label className="block text-xs font-bold text-stone-500 mb-1.5 uppercase">Ghi chú chốt sổ</label>
                                <input
                                    type="text"
                                    value={closeNotes}
                                    onChange={(e) => setCloseNotes(e.target.value)}
                                    placeholder="Không bắt buộc. Nhập ghi chú bàn giao cuối ngày..."
                                    className="w-full px-4 py-2.5 bg-stone-50 dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl text-sm"
                                />
                            </div>

                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-stone-200 dark:border-zinc-700">
                                <button onClick={() => setCloseShiftModal(false)} className="px-4 py-2 text-sm font-bold text-stone-600 hover:bg-stone-100 dark:hover:bg-zinc-700 rounded-2xl">
                                    Hủy
                                </button>
                                <button onClick={handleCloseShift} className="px-6 py-2 text-sm font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-2xl shadow-lg active:scale-95 transition-all">
                                    Xác nhận chốt sổ ca
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-fadeIn">
                        <div className="p-6">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Số lượng giao</label>
                                        <input
                                            type="number"
                                            step="any"
                                            value={editQtySent}
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">ĐVT</label>
                                        <select
                                            value={editUnitSent}
                                ...
                                {(editModal.result_quantity !== null || editModal.status !== 'sent') && (
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Số lượng nhận</label>
                                            <input
                                                type="number"
                                                step="any"
                                                value={editQtyResult || 0}
                                                onChange={(e) => setEditQtyResult(parseFloat(e.target.value) || 0)}
                                                className="w-full px-4 py-2.5 bg-stone-50 dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl text-sm"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">ĐVT nhận</label>
                                            <select
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Ghi chú</label>
                                    <input
                                        type="text"
                                        value={editNotes}
                            ...
                            <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-stone-200 dark:border-zinc-700">
                                <button onClick={() => setEditModal(null)} className="px-4 py-2 text-sm font-bold text-stone-600 hover:bg-stone-100 dark:hover:bg-zinc-700 rounded-2xl">
                                    Hủy
                                </button>
                                <button 
                                    onClick={handleSaveEdit} 
                                    disabled={isSaving}
                                    className="px-6 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-2xl shadow-lg shadow-indigo-500/20 active:scale-95 disabled:opacity-50 transition-all flex items-center gap-1"
                                >
            ...
            {openShiftModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white dark:bg-zinc-800 rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                        <div className="p-6">
                            <h3 className="text-lg font-bold text-stone-900 dark:text-white flex items-center gap-2 mb-4 text-emerald-600">
                                🟢 Bắt đầu Mở ca giao nhận
                            </h3>
                            <div className="mb-4">
                                <label className="block text-xs font-bold text-stone-500 mb-1.5 uppercase">Ghi chú mở ca</label>
                                <input
                                    type="text"
                                    value={openNotes}
                                    onChange={(e) => setOpenNotes(e.target.value)}
                                    placeholder="Ví dụ: Ca làm việc sáng, Nhân viên bàn giao..."
                                    className="w-full px-4 py-2.5 bg-stone-50 dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl text-sm"
                                />
                            </div>
                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-stone-200 dark:border-zinc-700">
                                <button onClick={() => setOpenShiftModal(false)} className="px-4 py-2 text-sm font-bold text-stone-600 hover:bg-stone-100 dark:hover:bg-zinc-700 rounded-2xl">
                                    Hủy
                                </button>
                                <button onClick={handleOpenShift} className="px-6 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-2xl shadow-lg active:scale-95 transition-all">
                                    Xác nhận Mở ca
                                </button>
                            </div>
                        </div>

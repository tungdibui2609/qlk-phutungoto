...
export default function WarehouseMapPrintPage() {
    ...
    // Editable titles (Moved from previous implementation)
    ...
    const [filterRows, setFilterRows] = useState<FilterRow[]>([{ id: Date.now().toString(), warehouseId: null, rowId: null, binId: null, levelId: null }])

    // --- State: Printing/Capture ---
    const [isDownloading, setIsDownloading] = useState(false)
    const { isCapturing, handleCapture, downloadTimer } = useCaptureReceipt({
        fileNamePrefix: `so-do-kho-${new Date().toISOString().split('T')[0]}`
    })
    const isSnapshotMode = isSnapshot || isCapturing

    // --- Hooks: Shared/External ---
    const { getBaseAmount: toBaseAmount, getBaseToKgRate } = useUnitConversion()
    ...
    const initialCompanyInfo = cmpName ? {
        name: cmpName, address: cmpAddress, phone: cmpPhone, email: cmpEmail, logo_url: cmpLogo, short_name: cmpShort, tax_code: null
    } : null
    ...
    const filteredPositions = useMemo(() => {
        ...
        // --- Hierarchical Filtering (Multi-row Support) ---
        if (displayInternalCode) {
            const hasActiveFilters = filterRows.some(r => r.warehouseId || r.rowId || r.binId || r.levelId)
            
            if (hasActiveFilters) {
                const descendantCache = new Map<string, Set<string>>()
                const getDescendants = (pid: string) => {
                    if (descendantCache.has(pid)) return descendantCache.get(pid)!
                    ...
                    collect(pid)
                    descendantCache.set(pid, descendants)
                    return descendants
                }

                result = result.filter(p => {
                    return filterRows.some(row => {
                        if (row.levelId) return p.zone_id === row.levelId
                        if (row.binId) {
                            const levelIds = displayZones.filter(z => z.parent_id === row.binId).map(z => z.id)
                            return p.zone_id === row.binId || (p.zone_id && levelIds.includes(p.zone_id))
                        }
                        if (row.rowId) {
                            const descendants = getDescendants(row.rowId)
                            return p.zone_id && descendants.has(p.zone_id)
                        }
                        if (row.warehouseId) {
                            const descendants = getDescendants(row.warehouseId)
                            return p.zone_id && descendants.has(p.zone_id)
                        }
                        return false
                    })
        ...
        // --- Manual Zone Selection Filter ---
        if (onlyShowChecked && checkedZoneIds.size > 0) {
            const descendantCache = new Map<string, Set<string>>()
            const getDescendants = (pid: string) => {
                if (descendantCache.has(pid)) return descendantCache.get(pid)!
                const descendants = new Set<string>()
            ...
            // A position is visible if it matches any checked zone or its descendants
            result = result.filter(p => {
                return Array.from(checkedZoneIds).some(checkedId => {
                    const descendants = getDescendants(checkedId)
                    return p.zone_id === checkedId || (p.zone_id && descendants.has(p.zone_id))
                })
            })
        }
    ...
    const filteredZones = useMemo(() => {
        ...
        if (displayInternalCode) {
            ...
            if (hasActiveFilters) {
                const combinedZones = new Set<string>()
                filterRows.forEach(row => {
                    const targetId = row.levelId || row.binId || row.rowId || row.warehouseId
                    if (!targetId) return

                    let curr: any = displayZones.find(z => z.id === targetId)
                    while (curr) {
                        combinedZones.add(curr.id)
                ...
                if (combinedZones.size > 0) {
                    result = result.filter(z => combinedZones.has(z.id))
                }
            }
        }
        ...
        if (onlyShowChecked && checkedZoneIds.size > 0) {
            ...
            Array.from(checkedZoneIds).forEach(checkedId => {
                visibleZones.add(checkedId)
                const descendants = getDescendants(checkedId)
                descendants.forEach(id => visibleZones.add(id))
        ...
        try {
            ...
            const fetchAll = async (table: string, filter?: (query: any) => any, select = '*', limit = 1000) => {
                ...
                if (['positions', 'zones', 'zone_layouts', 'lots'].includes(table)) {
                    ...
                    for (const item of allRecs) if (item.id) uniqueMap.set(item.id, item)
                    return Array.from(uniqueMap.values())
                } else if (table === 'zone_positions') {
                    const uniqueMap = new Map()
                    for (const item of allRecs) uniqueMap.set(`${item.zone_id}-${item.position_id}`, item)
                    return Array.from(uniqueMap.values())
                }
                return allRecs
            }
            ...
            zpData.forEach((zp: any) => { if (zp.position_id && zp.zone_id) zpLookup[zp.position_id] = zp.zone_id })

            const posWithZone: PositionWithZone[] = (posData as any[]).map(pos => ({ ...pos, zone_id: zpLookup[pos.id] || null }))
    ...
    useEffect(() => {
        if (displayInternalCode) {
            setIsEmptyMap(true)
            setSignTitle3('Sản Xuất')
        }
    }, [displayInternalCode])


    // --- Handlers: Printing & Export ---
    ...
    const handleToggleMergeZone = (zoneId: string) => {
        setMergedZones(prev => {
            const next = new Set(prev)
            if (next.has(zoneId)) next.delete(zoneId)
            else next.add(zoneId)
    ...
    const handleToggleCheckedZone = (zoneId: string, isChecked: boolean) => {
        setCheckedZoneIds(prev => {
            const next = new Set(prev)
            const toggleRecursive = (id: string, checked: boolean) => {
                if (checked) next.add(id)
                else next.delete(id)
                displayZones.filter(z => z.parent_id === id).forEach(z => toggleRecursive(z.id, checked))
            }
            toggleRecursive(zoneId, isChecked)
            return next
        })
    }

    // --- Handlers: Hierarchical Filters ---
    const addFilterRow = () => setFilterRows(prev => [...prev, { id: Date.now().toString(), warehouseId: null, rowId: null, binId: null, levelId: null }])
    const removeFilterRow = (id: string) => setFilterRows(prev => prev.length > 1 ? prev.filter(r => r.id !== id) : prev)
    const updateFilterRow = (id: string, updates: Partial<FilterRow>) => setFilterRows(prev => prev.map(r => r.id === id ? { ...r, ...updates } : r))

    // --- Handlers: Excel Exports ---
    ...
    const handleExportExcelTable = async () => {
        ...
        await exportWarehouseToExcel({ systemName: systemType || 'KHO', zoneName: displayZones.find(z => z.id === selectedZoneId)?.name, searchTerm: searchTerm, positions: excelPositions })
    }
    ...
    const handleExportExcelGrid = async () => {
        ...
        const roots = potentialRoots.length > 0 ? potentialRoots.filter(z => !potentialRoots.find(pz => pz.id === z.parent_id)) : filteredZones.filter(z => !z.parent_id || !filteredZones.find(pz => pz.id === z.parent_id));

        const getTopParentName = (zone: any): string => {
            let current = zone; let depth = 0;
        ...
        roots.forEach(root => {
            ...
            if (bins.length === 0) {
                ...
                collect(root.id)
                if (items.length > 0 || /S[ẢảÀà]nh/i.test(root.name)) grids.push({ name: root.name, parentName, bins: [], levels: [], cells: [{ binIndex: 0, levelIndex: 0, items }] })
                return
            }
            ...
            bins.forEach(bin => {
                ...
                if (subZones.length > 0) subZones.forEach(lvl => levelNamesSet.add(lvl.name))
                else if (filteredPositions.some(p => p.zone_id === bin.id)) levelNamesSet.add('DỮ LIỆU')
            })
            const sortedLevels = Array.from(levelNamesSet).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }))
            const cells: any[] = []
            bins.forEach((bin, binIdx) => {
                if (mergedZones.has(bin.id) && sortedLevels.length > 0) {
                    ...
                    collect(bin.id)
                    cells.push({ binIndex: binIdx, levelIndex: 0, items: Array.from(aggregated.values()), isMerged: true, rowSpan: sortedLevels.length })
                } else {
    ...
    const handleExportExcelLobbyDetail = async () => {
        ...
        if (lobbies.length === 0) return alert("Không tìm thấy sảnh nào trong khu vực hiện tại.")
        const lobbyData = lobbies.map(lobby => {
            const layout = layouts[lobby.id]; const columns = layout?.position_columns || 8;
            const sortedPos = sortPositionsByBinPriority(filteredPositions.filter(p => p.zone_id === lobby.id));
            const excelPositions = sortedPos.map((p, idx) => ({
                x: idx % columns, y: Math.floor(idx / columns), code: p.code,
                items: (p.lot_id ? lotInfo[p.lot_id] : null)?.items?.map((it: any) => ({ productName: displayInternalCode && it.internal_name ? it.internal_name : it.product_name, sku: displayInternalCode && it.internal_code ? it.internal_code : it.sku, unit: it.unit, quantity: it.quantity, lotCode: (p.lot_id ? lotInfo[p.lot_id] : null).code, batchCode: (p.lot_id ? lotInfo[p.lot_id] : null).batch_code, lotTags: it.tags })) || []
            }));
        ...
        <>
            ...
            <div className={`fixed top-4 right-4 z-50 print:hidden flex gap-3 items-center ${isSnapshotMode ? 'hidden' : ''}`}>
                    ...
                    {isSettingsOpen && (
                        <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col p-4 gap-6 animate-in fade-in slide-in-from-top-2 duration-200">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setIsZonePickerOpen(!isZonePickerOpen); }}
                                    ...
                                    >
                                        <span className="truncate flex-1 text-left">{displayZones.find(z => z.id === selectedZoneId)?.name || 'Tất cả Zone'}</span>
                                    </button>
                                        ...
                                        <div className="absolute right-full mr-2 top-0 w-80 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col max-h-[400px] z-[60]">
                                            <div className="p-2 border-b bg-gray-50/50">
                                                <div className="relative">
                                                    ...
                                                    <input type="text" placeholder="Tìm zone..." value={zoneSearchTerm} onChange={(e) => setZoneSearchTerm(e.target.value)} className="w-full pl-8 pr-2 py-1.5 bg-white border border-gray-200 rounded text-xs outline-none" autoFocus />
                                                </div>
                                            </div>
                                            <div className="overflow-y-auto p-1 custom-scrollbar">
                                                {(() => {
                                                    const baseZones = selectedZoneId && !zoneSearchTerm ? displayZones.filter(z => z.id === selectedZoneId) : displayZones.filter(z => !z.parent_id);
                                                    const toggleExpand = (e: React.MouseEvent, id: string) => { e.stopPropagation(); setExpandedZoneIds(prev => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; }) }
                                                    const childrenMap = new Map<string, Zone[]>()
                                                    displayZones.forEach(z => { if (z.parent_id) { const list = childrenMap.get(z.parent_id) || []; list.push(z); childrenMap.set(z.parent_id, list) } })

                                                    const renderTree = (z: Zone, depth: number): React.ReactNode[] => {
                                                        const subZones = childrenMap.get(z.id) || []; const hasChildren = subZones.length > 0; const isExpanded = expandedZoneIds.has(z.id) || zoneSearchTerm;
                                                        ...
                                                        if (zoneSearchTerm && !matchesSearch && hasChildren) {
                                                            const checkDescendants = (nodeId: string): boolean => {
                                                                ...
                                                                for (const child of children) { if (child.name.toLowerCase().includes(zoneSearchTerm.toLowerCase())) return true; if (checkDescendants(child.id)) return true }
                                                                return false
                                                            }
                                                        ...
                                                        return [<div key={z.id}>
                                                            <button onClick={() => { handleZoneChange(z.id); setIsZonePickerOpen(false); setZoneSearchTerm('') }} className={`w-full flex items-center gap-1.5 px-2 py-1.5 rounded text-xs hover:bg-blue-50 group ${selectedZoneId === z.id ? 'bg-blue-50 text-blue-700 font-bold' : 'text-gray-700'}`}>
                                                                <div className="shrink-0" style={{ width: depth * 12 }} />
                                                                {hasChildren ? <div onClick={(e) => toggleExpand(e, z.id)} className="p-0.5 hover:bg-gray-200 rounded text-gray-400">{isExpanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}</div> : <div className="w-[18px]" />}
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="grid grid-cols-3 gap-1 bg-gray-100 p-1 rounded-lg">
                                    {(['all', 'occupied', 'empty'] as const).map(f => (
                                        <button key={f} onClick={() => setOccupancyFilter(f)} className={`py-1.5 text-[11px] font-medium rounded-md transition-all cursor-pointer ${occupancyFilter === f ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-2">
                                    <label className="text-[11px] font-bold uppercase text-gray-400 flex items-center gap-2"><Monitor size={12} /> Khổ giấy</label>
                                    <div className="flex bg-gray-100 p-1 rounded-lg">
                                        <button onClick={() => handleOrientationChange('portrait')} className={`flex-1 py-1.5 text-[11px] font-medium rounded-md cursor-pointer ${orientation === 'portrait' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>Dọc</button>
                                        <button onClick={() => handleOrientationChange('landscape')} className={`flex-1 py-1.5 text-[11px] font-medium rounded-md cursor-pointer ${orientation === 'landscape' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>Ngang</button>
                                    </div>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label className="text-[11px] font-bold uppercase text-gray-400 flex items-center gap-2"><Layout size={12} /> Hiển thị</label>
                                    <div className="flex bg-gray-100 p-1 rounded-lg">
                                        <button onClick={() => setShowTable(false)} className={`flex-1 py-1.5 text-[11px] font-medium rounded-md cursor-pointer ${!showTable ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>Sơ đồ</button>
                                        <button onClick={() => setShowTable(true)} className={`flex-1 py-1.5 text-[11px] font-medium rounded-md cursor-pointer ${showTable ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>Bảng</button>
                                    </div>
                                    {!showTable && (
                                        <label className="flex items-center gap-3 cursor-pointer group mt-1">
                                            <div className="relative inline-flex items-center">
                                                <input type="checkbox" className="sr-only peer" checked={isEmptyMap} onChange={(e) => setIsEmptyMap(e.target.checked)} />
                                    ...
                                    {isEmptyMap && (
                                        <div className="space-y-2 mt-2 pt-2 border-t border-gray-100">
                                            <label className="flex items-center gap-3 cursor-pointer group">
                                                <div className="relative inline-flex items-center">
                                                    <input type="checkbox" className="sr-only peer" checked={onlyShowChecked} onChange={(e) => setOnlyShowChecked(e.target.checked)} />
                                                    <div className="w-8 h-4 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-purple-600"></div>
                                                </div>
                                                <span className="text-[11px] font-bold text-purple-700 group-hover:text-purple-800 transition-colors flex items-center gap-1.5">Chỉ hiện vùng đã chọn ({checkedZoneIds.size})</span>
                                            </label>
                                            {checkedZoneIds.size > 0 && (
                                                <button onClick={() => { setCheckedZoneIds(new Set()); setOnlyShowChecked(false); }} className="text-[10px] text-red-500 hover:text-red-700 font-medium underline flex items-center gap-1 ml-11"><X size={10} /> Bỏ chọn tất cả</button>
                                            )}
                                        </div>
                                    )}
                                </div>
                            ...
                            {/* 4. Grouping Toggle */}
                            <div className="flex flex-col gap-2">
                                <label className="text-[11px] font-bold uppercase text-gray-400 flex items-center gap-2"><Layers size={12} /> Chế độ gom ô</label>
                                <button onClick={() => setIsGrouped(!isGrouped)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg border transition-all cursor-pointer ${isGrouped ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
                                    <span className="text-xs font-semibold">{isGrouped ? 'Đang bật Gom ô' : 'Đang tắt Gom ô'}</span>
                                    <div className={`w-8 h-4 rounded-full relative transition-colors ${isGrouped ? 'bg-indigo-600' : 'bg-gray-300'}`}><div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${isGrouped ? 'left-4.5' : 'left-0.5'}`} /></div>
                                </button>
                            ...
                            {isGrouped && (
                                <div className="flex flex-col gap-2">
                                    <label className="text-[11px] font-bold uppercase text-gray-400 flex items-center gap-2"><Maximize2 size={12} /> Chế độ gộp tầng</label>
                                    <button onClick={() => {
                                        const { zones: displayZones } = groupWarehouseData(zones, positions)
                                        const mergeableZoneIds = displayZones.filter(z => { const nameUpper = z.name.toUpperCase(); const isSanh = nameUpper.startsWith('SẢNH') || nameUpper.startsWith('SÀNH') || nameUpper.startsWith('SANH'); return z.id.startsWith('v-lvl-') || z.id.startsWith('v-bin-') || z.name.startsWith('Ô ') || z.name.toUpperCase().startsWith('TẦNG ') || isSanh }).map(z => z.id)
                                        const allMerged = mergeableZoneIds.length > 0 && mergeableZoneIds.every(id => mergedZones.has(id))
                                        if (allMerged) setMergedZones(new Set()); else setMergedZones(prev => { const next = new Set(prev); mergeableZoneIds.forEach(id => next.add(id)); return next })
                                        ...
                                        <span className="text-xs font-semibold">{mergedZones.size > 0 ? 'Đang bật Gộp tầng' : 'Đang tắt Gộp tầng'}</span>
                                        <div className={`w-8 h-4 rounded-full relative transition-colors ${mergedZones.size > 0 ? 'bg-purple-600' : 'bg-gray-300'}`}><div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${mergedZones.size > 0 ? 'left-4.5' : 'left-0.5'}`} /></div>
                                    </button>
                                </div>
                            )}
                        </div>
                ...
                <div className="flex bg-white rounded-lg shadow-xl border border-gray-200 p-1 gap-1 flex-wrap">
                    ...
                    <button onClick={handleExportExcelLobbyDetail} className="px-4 py-2 bg-amber-600 text-white rounded-md text-sm font-bold flex items-center gap-2 hover:bg-amber-700 transition-colors cursor-pointer shadow-sm" disabled={isCapturing}><Monitor size={16} /> Excel (Sảnh)</button>
                    <button onClick={handlePrint} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-bold flex items-center gap-2 hover:bg-blue-700 transition-colors cursor-pointer shadow-sm" disabled={isCapturing}><Printer size={16} /> In sơ đồ</button>
                </div>
            </div>
            ...
            <div className={`min-h-screen print:min-h-0 bg-gray-100/50 print:bg-white py-8 print:py-0 ${isCapturing ? '!p-0 !bg-white' : ''}`}>
                ...
                `}} />
                    ...
                    {/* Hierarchical Filters for Empty Map (Screen Only) */}
                    {displayInternalCode && (
                        <div className="print:hidden mb-6 bg-blue-50/50 p-4 rounded-xl border border-blue-100 shadow-sm flex flex-col gap-3">
                            <div className="flex items-center justify-between mb-1">
                                <label className="text-xs font-bold uppercase text-blue-600 flex items-center gap-2">
                                    <Filter size={14} /> Bộ lọc vị trí in ({filterRows.length})
                                </label>
                                <button 
                                    onClick={addFilterRow}
                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700 transition-all shadow-sm cursor-pointer"
                                >
                                    <Plus size={14} /> Thêm vùng in
                                </button>
                            </div>

                            <div className="flex flex-col gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                {filterRows.map((row, index) => (
                                    <div key={row.id} className="relative flex flex-wrap gap-3 items-end bg-white p-3 rounded-lg border border-blue-100 shadow-sm animate-in fade-in slide-in-from-left-2 duration-200">
                                        <div className="absolute -left-2 -top-2 w-5 h-5 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-[10px] font-bold border border-blue-200">
                                            {index + 1}
                                        </div>

                                        <div className="flex-1 min-w-[140px]">
                                            <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Kho</label>
                                            <select 
                                                className="w-full bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all cursor-pointer"
                                                value={row.warehouseId || ''}
                                                onChange={(e) => updateFilterRow(row.id, { 
                                            ...
                                            >
                                                ...
                                                {displayZones.filter(z => !z.parent_id).map(z => (
                                        ...
                                        <div className="flex-1 min-w-[140px]">
                                            <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Dãy / Khu vực</label>
                                            <select 
                                                className="w-full bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all cursor-pointer disabled:opacity-50"
                                                value={row.rowId || ''}
                                                onChange={(e) => updateFilterRow(row.id, { 
                                            ...
                                            >
                                                ...
                                                {displayZones.filter(z => z.parent_id === row.warehouseId).map(z => (
                                                    <option key={z.id} value={z.id}>{z.name}</option>
                                                ))}
                                            </select>
                                        </div>
                                        ...
                                        <div className="flex-1 min-w-[140px]">
                                            <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Ô (Ô gộp)</label>
                                            <select 
                                                className="w-full bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all cursor-pointer disabled:opacity-50"
                                                value={row.binId || ''}
                                                onChange={(e) => updateFilterRow(row.id, { 
                                                ...
                                                disabled={!row.rowId}
                                            >
                                                <option value="">Tất cả Ô</option>
                                                {displayZones.filter(z => z.parent_id === row.rowId).map(z => (
                                                    <option key={z.id} value={z.id}>{z.name}</option>
                                                ))}
                                            </select>
                                        </div>
                                        ...
                                        <div className="flex-1 min-w-[140px]">
                                            <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Tầng</label>
                                            <select 
                                                className="w-full bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all cursor-pointer disabled:opacity-50"
                                                value={row.levelId || ''}
                                                onChange={(e) => updateFilterRow(row.id, { levelId: e.target.value || null })}
                                                disabled={!row.binId}
                                            >
                                                <option value="">Tất cả Tầng</option>
                                                {displayZones.filter(z => z.parent_id === row.binId).map(z => (
                                                    <option key={z.id} value={z.id}>{z.name}</option>
                                                ))}
                                            </select>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button 
                                                ...
                                                className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer disabled:opacity-30 disabled:hover:bg-transparent"
                                            ...
                                            <button 
                                                onClick={() => updateFilterRow(row.id, { warehouseId: null, rowId: null, binId: null, levelId: null })}
                                                className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer"
                                                title="Đặt lại bộ lọc này"
                                            >
                                                <RefreshCw size={14} />
                                            </button>
                                        </div>
                            ...
                            {filterRows.length > 1 && (
                                <div className="flex justify-end pt-2 border-t border-blue-100">
                                    <button 
                                        onClick={() => setFilterRows([{ id: Date.now().toString(), warehouseId: null, rowId: null, binId: null, levelId: null }])}

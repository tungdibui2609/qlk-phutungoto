...
export async function POST(request: NextRequest) {
    try {
        ...
        const hasOutboundFinancials = (() => {
            ...
            return Array.isArray(modules) && modules.includes('outbound_financials')
        })()

        const hasOutboundConversion = (() => {
            if (!systemConfig) return false
            const modules = typeof systemConfig.outbound_modules === 'string'
                ? JSON.parse(systemConfig.outbound_modules)
                : systemConfig.outbound_modules
            return Array.isArray(modules) && modules.includes('outbound_conversion')
        })()
        ...
        // Read template file from public folder
        ...
        for (const order of orders) {
                ...
                row.eachCell({ includeEmpty: false }, (cell) => {
                    if (cell.value && cell === cell.master) {
                        const replacePlaceholders = (text: string): string => {
                            ...
                            val = val.replace(/{{\s*KHACHHANG\s*}}/gi, order.customer_name || '')
                            val = val.replace(/{{\s*diachikhachhang\s*}}/gi, order.customer_address || '')
            ...
            exportItems.forEach((item: any, index: number) => {
                ...
                if (hasOutboundFinancials && isOfficial) {
            ...
            if (hasOutboundFinancials && isOfficial && totalAmount > 0) {
        ...
        return new NextResponse(zipBuffer, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename="Phieu_Xuat_${orders.length}_phieu.zip"`,

...
import { Plus, Trash2, Package, Sparkles, Loader2, RefreshCw, ClipboardList } from 'lucide-react'
...
export default function FinishedLotsPage() {
    ...
    // Load danh sách lô thành phẩm tự khai báo
    const fetchLots = async () => {
        ...
        try {
            ...
            if (err.message === 'TABLE_NOT_EXIST') {
                const localLots = localStorage.getItem(`local_custom_finished_lots_${currentSystem.code}`)
                ...
                console.error('Lỗi khi tải danh sách LOT thành phẩm:', err)
    ...
    const handleCreateLot = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast(`Đã tạo thành công lô thành phẩm: ${cleanCode}`, 'success')
            setNewLotCode('')
            fetchLots() // Refresh list
        } catch (err: any) {
            if (err.message === 'TABLE_NOT_EXIST') {
                const localKey = `local_custom_finished_lots_${currentSystem.code}`
                ...
                showToast(`Đã lưu tạm thời lô thành phẩm: ${cleanCode}`, 'success')
                setNewLotCode('')
                fetchLots()
            } else {
                console.error('Lỗi khi lưu LOT thành phẩm:', err)
    ...
    // Xử lý xóa lô thành phẩm
    const handleDeleteLot = async (id: string, code: string) => {
        if (!confirm(`Bạn có chắc chắn muốn xóa lô thành phẩm ${code}?`)) return
        ...
        try {
            if (id.startsWith('local-')) {
                ...
                showToast(`Đã xóa lô thành phẩm: ${code}`, 'success')
                fetchLots()
                return
            }
            ...
            console.error('Lỗi khi xóa LOT thành phẩm:', err)
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-stone-500 dark:text-stone-400 mt-1">
                        Khai báo nhanh các mã lô thành phẩm để sử dụng trong in ấn và truy xuất nguồn gốc thùng.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                >
                    <div className="flex items-center gap-2 border-b border-stone-100 dark:border-zinc-800 pb-3">
                        ...
                        <h3 className="font-bold text-stone-800 dark:text-white">Khai báo lô mới</h3>
                    </div>

                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">
                                Mã lô thành phẩm <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                value={newLotCode}
                                onChange={(e) => setNewLotCode(e.target.value)}
                                placeholder="Ví dụ: LÔ-TP-BUOI-01"
                                disabled={isSaving}
                ...
                <div className="lg:col-span-8 bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-stone-200 dark:border-zinc-800 shadow-sm space-y-4">
                    <div className="flex items-center justify-between border-b border-stone-100 dark:border-zinc-800 pb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-bold text-stone-800 dark:text-white">Danh sách lô thành phẩm</h3>
                        </div>
                        <button 
                            onClick={fetchLots}
                    ...
                    {isLoading ? (
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-stone-100 dark:border-zinc-800 text-[11px] font-black uppercase tracking-wider text-stone-400">
                                        <th className="py-3 text-left">Mã Lô Thành Phẩm</th>
                        ...
                        <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-stone-200 dark:border-zinc-800 rounded-2xl text-center space-y-3 bg-stone-50/30 dark:bg-zinc-900/30">
                            <Package className="text-stone-300 dark:text-zinc-700" size={48} />
                            <div>
                                <h4 className="font-bold text-stone-700 dark:text-zinc-300">Chưa có lô thành phẩm</h4>
                                <p className="text-xs text-stone-400 dark:text-zinc-500 mt-1 max-w-sm">
                                    Sử dụng biểu mẫu bên trái để khai báo nhanh mã lô thành phẩm đầu tiên của bạn.
                                </p>
                            </div>
                        </div>
                    )}

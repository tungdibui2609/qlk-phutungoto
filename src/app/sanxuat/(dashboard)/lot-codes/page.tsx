...
export default function SanxuatLotCodesPage() {
    ...
    const displayedTags = useMemo(() => {
        const uniqueNames = Array.from(new Set(masterTags.map(t => t.name)))
        const list = uniqueNames.sort()
    ...
    return (
        <section className="space-y-6 pb-12 animate-fade-in pl-4 pr-4 pt-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-zinc-900 dark:text-zinc-100 tracking-tight flex items-center gap-2">
                        Quản lý Mã Phụ Sản Xuất
                    </h1>
                    <p className="text-zinc-500 dark:text-zinc-400 mt-1">
                        Danh sách các mã phụ (Tags) dùng để phân loại LOT Sản Xuất.
                    </p>
                </div>
                <button
                    onClick={loadData}
            ...
            {/* Create Box */}
            <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm border border-emerald-100 dark:border-emerald-900/40">
                <h3 className="text-sm font-semibold text-emerald-800 dark:text-emerald-400 uppercase tracking-wider mb-4">
                    Thêm Mã Mới
                </h3>
                ...
                <p className="mt-2 text-xs text-zinc-500">
                    Tạo các mã đơn lẻ (Master Tags) để sử dụng khi xây dựng chuỗi mã phụ cho Lot.
                </p>
            </div>
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {displayedTags.length === 0 ? (
                            ...
                            displayedTags.map((tag, idx) => (
                                <div key={`${tag}-${idx}`} className="flex items-center justify-between p-4 hover:bg-emerald-50/50 dark:hover:bg-emerald-900/10 transition">

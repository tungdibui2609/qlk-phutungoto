...
import { Settings, Plus, Save, Trash2, PackageOpen, RefreshCw, AlertTriangle, CheckCircle, ArrowRightLeft, Factory } from 'lucide-react'
...
export default function SanxuatDeliverySettingsPage() {
    ...
    const loadMOList = useCallback(async () => {
        ...
        try {
            ...
            if (error) {
                console.error('Sanxuat - Supabase query error:', JSON.stringify(error))
                throw new Error(error.message || error.details || 'Unknown query error')
            }
            setProductionList(data || [])
        } catch (err: any) {
            console.error('Sanxuat - Load Production error:', err?.message || err)
            try {
                ...
                console.error('Sanxuat - Fallback error:', fbErr)
            }
        } finally {
            setMoLoading(false)
        }
    ...
    const toggleLotDirection = (lotId: string) => {
        setLotDirections(prev => {
            ...
            return { ...prev, [lotId]: 'warehouse_to_production' }
        })
    }
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">
                        Quản lý cấu hình sản phẩm giao/nhận (Sản xuất)
                    </p>
                </div>
                <button
                    onClick={loadMOList}

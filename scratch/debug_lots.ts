...
async function debugLots() {
    console.log('Fetching 1 row from lots with relationships...')
    const { data, error } = await supabase.from('lots').select(`
        id, code, production_code, production_lot_id, production_id
    `).limit(1)
    
    if (error) {
        console.error('Error:', JSON.stringify(error, null, 2))
    } else {
        console.log('Sample lot:', JSON.stringify(data?.[0], null, 2))
    }
}
